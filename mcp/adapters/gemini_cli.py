...
class GeminiCliAdapter(ClientAdapter):
    """Adapter for the Gemini CLI user-scope config (~/.gemini/settings.json).

    Gemini CLI reads global MCP servers from the ``mcpServers`` key of
    ``~/.gemini/settings.json`` — available in every project. Like Claude
    Code's ``~/.claude.json``, that file also holds unrelated CLI settings, so
    this adapter only ever touches the entry it manages and never deletes the
    file on removal.
    """

    _CONFIG_ENV_NAME = "GEMINI_CLI_CONFIG_PATH"
    ...
    def locate(self, environment: ExecutionEnvironment) -> LocationResult:
        ...
        return LocationResult(
            available=True,
            path=environment.home / ".gemini" / "settings.json",
            evidence=["Using the Gemini CLI user-scope config location (~/.gemini/settings.json)."],
        )
    ...
    def inspect(self, path: Path, server_name: str) -> AdapterInspection:
        ...
        try:
            ...
            finding = Finding(
                ...
                message="Gemini CLI configuration is not valid JSON.",
                scope={"path": str(path)},
                evidence=[str(exc)],
        ...
        if not isinstance(document, dict):
            finding = Finding(
                code="invalid_client_config",
                severity=Severity.ERROR,
                message="Gemini CLI configuration must be a JSON object.",
                scope={"path": str(path)},
    ...
    def render_removal(self, inspection: AdapterInspection, server_name: str) -> RenderResult:
        ...
        # Never delete ~/.gemini/settings.json: it holds unrelated CLI settings.
        content = json.dumps(document, indent=2) + "\n"
        return RenderResult(
            path=inspection.path,
            content=content,
        ...
        try:
            ...
            return [
                Finding(
                    ...
                    severity=Severity.ERROR,
                    message="Rendered Gemini CLI configuration is not valid JSON.",
    ...
    def activation_instructions(self) -> list[NextAction]:
        return [
            NextAction(
                kind="restart_client",
                message="Start a new Gemini CLI session (or run /mcp in an existing one) to load the updated MCP configuration.",
            )
        ]
