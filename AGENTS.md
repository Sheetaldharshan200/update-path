...
| `EXAKIT_INSTALL_PODMAN=1` | Linux and WSL only: let the installer install Podman when it is missing, without asking. It runs one package-manager command through `sudo` (apt, dnf, yum, zypper, pacman or apk) between the launcher step and the database step, and on Debian/Ubuntu brings `uidmap` with it. Unattended runs must opt in, because it is the one thing in the kit that changes the system rather than your home directory; `0` refuses and prints the command instead |
