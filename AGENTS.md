...
| `EXAKIT_RUNTIME=personal\|nano` | Which database runtime a **fresh** install deploys. The default is **Exasol Personal**, everywhere: macOS, native Linux (Podman required — the gate checks and names it) and Windows x86_64 (host Podman; the launcher installs Podman itself when missing — winget, possibly an administrator prompt). A platform Personal does not support (WSL, Windows arm64) **exits gracefully naming the support matrix** — never a silent reroute. `EXAKIT_RUNTIME=nano` is the explicit escape hatch that installs the Exasol Nano container instead; an installed kit's runtime is never changed by this variable |
