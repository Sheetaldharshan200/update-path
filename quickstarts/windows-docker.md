...
- **No Docker.** The database runs through Podman's default machine; if Podman is missing, the Exasol launcher offers to install it with Windows Package Manager — that install may ask for administrator approval, and it is the only step that ever does
...
| Windows on ARM (Snapdragon / Copilot+ PC) | Exasol Personal has no Windows arm64 deployment, and the installer exits saying exactly that. The container runtime is still available explicitly — `$env:EXAKIT_RUNTIME = 'nano'` with Docker Desktop — but exapump publishes a Windows build for x86_64 only, so that path gives you a working, empty database driven from a SQL client or pyexasol: no sample data, no `exakit sql`, and no AI bridge. For the full kit on this machine, use a Linux VM — the Linux arm64 builds do exist |
