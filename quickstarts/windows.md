...
| Windows on ARM (Snapdragon / Copilot+ PC) | Exasol Personal has no Windows arm64 deployment, and the installer exits saying exactly that. The legacy container runtime is still available explicitly — `$env:EXAKIT_RUNTIME = 'nano'`, which needs a container engine you install yourself — but exapump publishes a Windows build for x86_64 only, so that path gives you a working, empty database driven from a SQL client or pyexasol: no sample data, no `exakit sql`, and no AI bridge. For the full kit on this machine, use a Linux VM — the Linux arm64 builds do exist |
