...
| Issue | Fix |
|---|---|
| The Podman install asked for administrator approval | That is Windows Package Manager installing Podman, once, and the installer says so before it happens. Approve it, or install Podman yourself first (`winget install RedHat.Podman`, a reboot may be needed) and re-run — the kit itself never needs admin rights. If the approval is refused, the kit names Podman as the cause rather than the database, skips the database step and carries on: everything that does not need a database still installs, and `exakit update` finishes the job afterwards |
| "Podman is installed, but it cannot run containers" | Podman on Windows is a Linux machine under WSL, and after a reboot it is off. The kit asks `podman info` before deploying and starts a machine that is merely stopped; if it still cannot answer, fix what `podman info` reports (`podman machine start`, or `podman machine init` if there is no machine at all) and run `exakit update` |
...
| A step failed but the install continued | By design. The summary at the end names each missing piece and the one command that installs it. That now includes the database step itself: declining to reuse a running database, declining to delete a stopped one, a foreign process on port 8563 and a deploy that could not finish are recorded rather than fatal |
| "Reusing the existing Exasol deployment (started)" and then nothing | Fixed. The launcher accepts a start for a deployment it has only initialized and then does nothing; the kit now asks the database rather than the launcher's record, runs the launcher's own deploy when nothing answers, and asks again |
