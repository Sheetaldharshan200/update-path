...
# A release candidate, and candidates get deleted: rc2 and rc3 were both
# removed upstream while pinned, and rc3 reached a user as a 404 mid-install.
# A candidate at all, rather than 2.2, because the flipped Windows default
# needs a launcher with Windows local deployments, which no 2.2 release has.
# Twin of EXAKIT_PERSONAL_VERSION_FALLBACK in common.sh; both move together,
# and both go to 2.3.0 final the day it publishes.
$script:PersonalVersionFallback = "2.3.0-rc6"
